...
const standardizePhoneNumbers = arr => {
  let newArr = arr.map(current => {
    return current.match(/\d/g);
  });
  newArr = newArr.map(current => {
    return current.join("");
  });
  return newArr;
};
...
const unenrollBrook = arr => {
  const newArr = [];
  arr.forEach(current => {
    const tempArr = current.filter(cur => {
      return !cur.includes("Brook");
    });
    newArr.push(tempArr);
  });
  return newArr;
...
Return a two-dimensional array that organizes those strings based on the day on which they occur. For example, all 
events on Monday are in the first array, all events on Tuesday are in the second array, etc.
...
const sortByDay = arr => {
  const output = [[], [], [], [], [], [], []];
  arr.forEach(current => {
    if (current.includes("Monday")) output[0].push(current);
    if (current.includes("Tuesday")) output[1].push(current);
    if (current.includes("Wednesday")) output[2].push(current);
    if (current.includes("Thursday")) output[3].push(current);
    if (current.includes("Friday")) output[4].push(current);
    if (current.includes("Saturday")) output[5].push(current);
    if (current.includes("Sunday")) output[6].push(current);
  });
  return output;
};
