...
const totalSum = input => {
  let total = 0;
  input.forEach(arr => {
    const count = arr.reduce((acc, cur) => acc + cur);
    total += count;
  });
  return total;
};
...
const divisibleByFiveTwoToThePower = input => {
  const filteredInput = input.map(current => {
    return current.filter(cur => {
      return typeof cur === "number" && cur % 5 === 0;
    });
  });
  const output = filteredInput.map(current =>
    current.map(cur => {
      return Math.pow(2, cur);
    })
  );
  return output;
};
...
let findMaleAndFemale = data => {
  ...
  data.forEach(current => {
    if (current.gender === "female" || current.gender === "male") {
      if (names === "") {
        names += current.name;
      } else {
        names += " and " + current.name;
      }
    }
  });
  return names;
...
let findShortest = data => {
  const shortest = data.reduce((acc, current) => {
    if (acc === "" || current.name.length <= acc.length) {
      return current.name;
    }
    return acc;
  }, "");
